 ...
 ****************************************************************************/
PlannedStmt *
qp_planner(Query *parse, int cursorOptions, ParamListInfo boundParams, MockPath *mockpath)
{
	PlannedStmt *result;
	PlannerGlobal *glob;
	double		tuple_fraction;
  ...
  /* Determine what fraction of the plan is likely to be scanned */
	...
	/* TODO: primary planning entry point */
	top_plan = qp_plan_create(glob, parse, 
								false, tuple_fraction, &root, mockpath);
 ...
 * parse is the querytree produced by the parse analysis.
 * mockpath is the mock path produced by the parse analysis.
 ...
 */
Plan *
qp_plan_create(PlannerGlobal *glob, Query *parse, bool hasRecursion, 
						double tuple_fraction,PlannerInfo **subroot, MockPath *mockpath)
{
	...
	bool		hasOuterJoins;
	ListCell *l;
	int num_old_subplans =0;
	List *newHaving;	

	/* Create a PlannerInfo data structure*/
  ...
  root->hasRecursion = hasRecursion;
	 ...
	 *
	 * We record this information in the mockpath structure during the raw parse
	 */
	root->hasJoinRTEs = mockpath->hasJoinRTEs;
	hasOuterJoins = mockpath->hasOuterJoins;
	/*
	...
	*/
	 ...
	 *
	 * Actually our query plan should be able to skip this steps
	 */
	 ...
	 * preprocessing.
	 */
	/* TODO: How should I modify this part */
	if (hasOuterJoins)
	...
	{
		plan = qp_grouping_planner(root, tuple_fraction, mockpath);
		
		if (parse->commandType != CMD_SELECT && parse->commandType != CMD_QUERYPLAN)
		{
			List	   *returningLists;
			List	   *rowMarks;
 ...
 */
Plan *
qp_grouping_planner(PlannerInfo *root, double tuple_fraction, MockPath *mockpath)
{
	...
	if (parse->setOperations)
	{
		 ...
		 * original tlist.
		 */
		Assert(parse->commandType == CMD_QUERYPLAN);
	...
	else
	{
		...
		/*
		 * Generate the best path for this Query 
		 */
		best_path = path_create(root, sub_tlist, tuple_fraction, limit_tuples, mockpath, &dNumGroups);
		...
		if (best_path->parent)
		{
			path_rows = best_path->parent->rows;
			path_width = best_path->parent->width;
		}
		else
		{
