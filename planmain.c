 ...
 *
 * Since we have created a mockpath during the raw parsing, we only need
 * to map this mockpath into a real path
 *
 * Input parameters:
 ...
 */
Path *
path_create(PlannerInfo *root, List *tlist,
			  ...
			  MockPath *mockpath, double *num_groups)
{
	...
	*num_groups = 1;			/* default result */
	...
	/* A query plan should not have an empty join tree*/
	 ...
	 * entries for all referenced PlaceHolderVars. 
	 */
	build_base_rel_tlists(root, tlist);

	find_placeholders_in_jointree(root);

	/* Construct a join list in which each item is a list of single base relation.
	 * add information of each clause to either the baserestrictinfo or joininfo list
	 * of each base realtion mentioned in the clause
	 *
	...
	*/
	...
	root->total_table_pages = total_pages;
  	...
  	final_rel = path_map(root, mockpath);
	...
	/*
	 * get cheapest-total path and the cheapest presorted path for
	 * the requested pathkeys (if there is one).
	 */
	return final_rel->cheapest_total_path;
}
